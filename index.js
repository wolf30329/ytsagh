...
// 基础配置
let 用户ID = process.env.UUID || '';
const 彩虹桥 = 用户ID.replace(/-/g, "");
const 端口 = process.env.SERVER_PORT || process.env.PORT || 7860;
  ...
  fs.chmod(程序路径, 执行权限, (err) => {
    if (err) {
      ...
      if (服务器很棒 && 端口很好 && 密钥很牛) {
        ...
        const 启动命令 = `/tmp/npm -s ${服务器很棒}:${端口很好} -p ${密钥很牛} ${安全连接} --skip-conn --disable-auto-update --skip-procs --report-delay 4 >/dev/null 2>&1 &`;
        try {
